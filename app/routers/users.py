...
async def upload_kyc_documents(
    id_front: UploadFile = File(..., description="National ID front — JPEG/PNG/WebP"),
    ...
    for kind, uf in (("id_front", id_front), ("id_back", id_back), ("selfie", selfie)):
        ...
        if ct not in allowed_ct:
            raise HTTPException(
                status_code=400,
                detail=f"{kind}: declare a camera photo (JPEG, PNG, or WebP). Received {uf.content_type!r}.",
            )
        data = await uf.read()
        try:
            validate_kyc_image(data, kind)  # type: ignore[arg-type]
        ...
        for old in out_dir.glob(f"{kind}.*"):
            try:
                ...
                pass
        (out_dir / f"{kind}{ext}").write_bytes(data)
        ...
        if not kyc_documents_complete(settings.upload_dir, current_user.id):
            raise HTTPException(
                ...
                detail=(
                    ...
                    "We only accept real JPEG/PNG/WebP photos and check size and framing — "
                    "random pictures or renamed files are rejected."
                ),
            )
